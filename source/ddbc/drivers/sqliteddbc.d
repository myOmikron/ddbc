...
version (USE_SQLITE)
{
    ...
    //  YYYY-MM-DD HH:MM:SS
    //  YYYYMMDDHHMMSS
    private SysTime parseSysTime(S)(in S sqliteString) @safe if (isSomeString!S)
    {
        //
        try
        {
            ...
            else if (match(sqliteString, r".*[\+|\-]\d{1,2}:\d{1,2}|.*Z"))
            {
                return sqliteString.canFind('-')
                    ? SysTime.fromISOExtString(sqliteString) : SysTime.fromISOString(sqliteString);
            }
            else
            {
    ...
    private DateTime fromResultSet(S)(in S sqliteString) @safe if (isSomeString!S)
    {
        ...
        try
        {
            ...
            case 23:
                    ...
                    }))
                {
                    ...
                    {
                        sharedLog.warning(sqliteString
                                ~ " will be converted to DateTime and lose the milliseconds. Consider using SysTime");
                    }
                }
    ...
    unittest
    {
        ...
        assert(ymdthmssUtc.toISOExtString() == "2019-09-15T15:18:51.5Z",
                ymdthmssUtc.toISOExtString());

        SysTime ymdthmssUtcPlus2 = parseSysTime("2019-09-15T15:18:51.500+02:00"); // YYYY-MM-DDTHH:MM:SS.SSS+HH:MM
        assert(ymdthmssUtcPlus2.toISOExtString() == "2019-09-15T15:18:51.5+02:00",
                ymdthmssUtcPlus2.toISOExtString());
    }
    ...
    class SQLITEPreparedStatement : SQLITEStatement, PreparedStatement
    {
        ...
        this(SQLITEConnection conn, string query)
        {
            ...
            int res = sqlite3_prepare_v2(conn.getConnection(), /* Database handle */
                    toStringz(query), /* SQL statement, UTF-8 encoded */
                    cast(int) query.length, /* Maximum length of zSql in bytes. */
                    &stmt, /* OUT: Statement handle */
                    null  /* OUT: Pointer to unused portion of zSql */
            );
            enforceHelper!SQLException(res == SQLITE_OK,
                    "Error #" ~ to!string(
                        res) ~ " while preparing statement " ~ query ~ " : " ~ conn.getError());
        ...
        private void allParamsSet()
        {
            ...
            {
                enforceHelper!SQLException(paramIsSet[i],
                        "Parameter " ~ to!string(i + 1) ~ " is not set");
            }
            if (preparing)
            {
                preparing = false;
    ...
    public:
        ...
        override void close()
        {
            ...
            enforceHelper!SQLException(res == SQLITE_OK,
                    "Error #" ~ to!string(
                        res) ~ " while closing prepared statement " ~ query ~ " : " ~ conn.getError());
            closed = true;
            conn.onStatementClosed(this);
        }
        ...
        override int executeUpdate(out Variant insertId)
        {
            ...
            {
                enforceHelper!SQLException(false,
                        "Error #" ~ to!string(
                            res) ~ " while trying to execute prepared statement: " ~ " : " ~ conn.getError());
            }
            return rowsAffected;
        }
        ...
        override ddbc.core.ResultSet executeQuery()
        {
            ...
            scope (exit)
                unlock();
            allParamsSet();
            enforceHelper!SQLException(metadata.getColumnCount() > 0,
                    "Query doesn't return result set");
        ...
        override void setBytes(int parameterIndex, byte[] x)
        {
            ...
            sqlite3_bind_blob(stmt, parameterIndex, cast(const(void*)) x.ptr,
                    cast(int) x.length, SQLITE_TRANSIENT);
            paramIsSet[parameterIndex - 1] = true;
        }
        ...
        override void setUbytes(int parameterIndex, ubyte[] x)
        {
            ...
            sqlite3_bind_blob(stmt, parameterIndex, cast(const char*) x.ptr,
                    cast(int) x.length, SQLITE_TRANSIENT);
            paramIsSet[parameterIndex - 1] = true;
        }
        ...
        override void setString(int parameterIndex, string x)
        {
            ...
            sqlite3_bind_text(stmt, parameterIndex, cast(const char*) x.ptr,
                    cast(int) x.length, SQLITE_TRANSIENT);
            paramIsSet[parameterIndex - 1] = true;
        }
    ...
    class SQLITEResultSet : ResultSetImpl
    {
        ...
        {
            enforceHelper!SQLException(columnIndex >= 1 && columnIndex <= columnCount,
        ...
        if (SQLITE_TESTS_ENABLED)
        {
            ...
            }
            {
                ...
                scope (exit)
                    stmt.close();
                stmt.executeUpdate(
                        "CREATE TABLE IF NOT EXISTS t1 (id INTEGER PRIMARY KEY, name VARCHAR(255) NOT NULL, flags int null)");
            }
            {
            ...
            }
            {
                ...
                PreparedStatement stmt = conn.prepareStatement(
                        "SELECT id, name, flags FROM t1 WHERE id = ?");
                scope (exit)
                    stmt.close();
    ...
    __gshared static this()
    {
        ...
        DriverFactory.registerDriverFactory("sqlite", delegate() {
